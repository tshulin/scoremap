// Sync layer - builds everything the app shows from the bundled sample data
// (src/data/testAccount.js and displayAccount.js) and maps it to the page
// shapes. A sync makes no
// requests: it assembles the active sample account's snapshot and returns it.
...
function mapGradebook(rawGradebook) {
  ...
  for (const course of gradebook.courses) {
    ...
    classes.push({
      ...
      // The sample has category weights we could not read - calc/ falls
...
// The resources a sync can fetch. Nothing is fetched any more - every one of
// them is built from bundled sample data - but the scope argument is kept so
// the pages that refresh a single section still work unchanged.
export const ALL_RESOURCES = ['gradebook', 'attendance', 'documents', 'mail'];

// Build the snapshot for whichever sample account is active. There is no
// network here and nothing to fail, so the progressive callbacks
// (`onUpdate`, `onStage`, `onGrades`) all fire once,
// immediately, with the finished snapshot.
export async function sync(knownStudent, { onUpdate, onStage, onGrades } = {}) {
  const snapshot = DEMO
    ? demoSnapshot()
    : api.builtinAccount() === 'display'
      ? displaySnapshot()
      : testSnapshot();
