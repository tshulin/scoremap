...
import { TEST_MAIL } from './testAccount.js';
import {
	DISPLAY_ATTENDANCE,
	DISPLAY_DOCUMENTS,
	DISPLAY_GRADEBOOK,
...
describe('DISPLAY_GRADEBOOK', () => {
	...
	it('the stored percentage matches what calc/ computes', () => {
		for (const course of DISPLAY_GRADEBOOK.courses) {
			for (const mark of course.marks) {
				...
				expect(
					...
					`${course.name} ${mark.name}: stored ${mark.percentage}, computed ${computed}`
				).toBe(true);
			}
		}
	});
...
describe('DISPLAY_MAIL', () => {
	...
	it('shares no senders with the second fictional mailbox', () => {
		...
		for (const message of DISPLAY_MAIL) {
			...
			expect(message.sender.email.endsWith('@example.edu'), message.sender.email).toBe(true);
		}
	});
