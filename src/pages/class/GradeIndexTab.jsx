// The class's letter scale: cutoffs observed from the sample's stored letters,
// defaults where nothing has been seen, and student overrides on top. The
// letters may use a teacher-specific scale - we reconstruct the scale
  ...
  return (
    ...
    >
      <div style={{ fontSize: 14, color: 'var(--color-body)', marginBottom: 4 }}>
        {observedCount > 0
          ? `Cutoffs inferred from ${observedCount} stored letter${observedCount === 1 ? '' : 's'} for this sample class, on top of the standard scale.`
          : 'No stored letters exist for this sample class yet. This is the standard scale.'}
      </div>
      <div style={{ fontSize: 13, color: 'var(--color-muted)', marginBottom: 14 }}>
        ...
        scale. These letters label the hypothetical grade and both calculators. The sample's
        stored letter is always shown as-is.
      </div>
