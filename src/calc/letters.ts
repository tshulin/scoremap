// Per-class letter cutoffs. Each sample class can use a teacher-specific scale
// server-side and only shows us the results, so cutoffs are OBSERVED from
// (percentage, letter) pairs stored in the data, inferred conservatively, and
// overridable by the student. GradeCompass never computed cutoffs either - it
// displayed stored letters; this reconstructs the scale they imply.
...
// A letter paired with 0% (or worse) is never evidence of a teacher's scale -
// it's a landing-page row whose letter arrived without a percentage (parsed as
// 0), and trusting one drags that letter's bound to the floor forever.
const isEvidence = (o: Observation): boolean => Number.isFinite(o.pct) && o.pct > 0;
...
export function inferScale(
	...
	for (const o of observations) {
		...
		if (!isLetter(letter) || !isEvidence(o)) continue;
