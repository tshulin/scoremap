...
// sessionStorage: the display (sample) account is tab-scoped by design - a
// reload keeps the demo going, but a fresh visit never auto signs in to it.
const DISPLAY_SESSION_KEY = 'grademax-display-session';
...
(function migrateStorage() {
  try {
    ...
    // The display account no longer auto signs in; retire a persisted marker
    // (and the sample data mirrored next to it) left by earlier versions.
    if (localStorage.getItem(TEST_SESSION_KEY) === 'display') {
...
// A built-in account has no cookie jar; a marker lets a later load resume it.
// The test account's marker lives in localStorage ('true', predating the
// display account) and auto signs in like stored credentials do. The display
// (sample) account's marker is deliberately tab-scoped: only actual accounts
// get auto sign-in.
export function isTestSession() {
  if (builtinSession) return true;
  try {
    if (localStorage.getItem(TEST_SESSION_KEY) === 'true') builtinSession = 'test';
    else if (sessionStorage.getItem(DISPLAY_SESSION_KEY) === 'true') builtinSession = 'display';
  } catch {
    /* storage unavailable */
  }
...
export async function login({ domain, username, password }) {
  ...
  if (builtin) {
    ...
    try {
      if (builtin === 'test') localStorage.setItem(TEST_SESSION_KEY, 'true');
      else sessionStorage.setItem(DISPLAY_SESSION_KEY, 'true');
    } catch {
